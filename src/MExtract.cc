...
#include "pfs/drp/stella/MExtract.h"
...
int optextract(const string &fitsFileName_In,
  ...
  cout << "USAGE: optextract <char[] (@)FitsFileName_In> <char[] (@)DatabaseFileName_In> <char[] (@)FitsFileName_Out> <double Gain> <double ReadOutNoise> [TELLURIC=int[0 - none, 1 - Piskunov, 2 - LinFit]] [MAX_ITER_SF=int] [MAX_ITER_SKY=int] [MAX_ITER_SIG=int] [SWATH_WIDTH=int] [SMOOTH_SF=double] [SMOOTH_SP=int] [WING_SMOOTH_FACTOR=double] [ERR_IN=char[](@)] [ERR_OUT_2D=char[](@)] [ERR_OUT_EC=char[](@)] [ERRFIT_OUT_EC=char[](@)] [SKY_OUT_EC=char[](@)][SKYFIT_OUT_EC=char[](@)] [SKY_OUT_2D=char[](@)] [SKYFIT_OUT_2D=char[](@)] [SKY_ERR_OUT_EC=char[](@)] [SKYFIT_ERR_OUT_EC=char[](@)] [PROFILE_OUT=char[](@)] [IM_REC_OUT=char[](@)] [REC_FIT_OUT=char[](@)] [MASK_OUT=char[](@)] [SPFIT_OUT_EC=char[](@)] [EC_FROM_PROFILE_OUT=char[](@)] [AREA=[int(xmin),int(xmax),int(ymin),int(ymax)]] [XCOR_PROF=int] [APERTURES=char[](@)]" << endl;//"[, ERR_FROM_PROFILE_OUT=char[]])" << endl;
  ...
  cout << "MExtract::main: gain_In set to " << gain_In << endl;
  cout << "MExtract::main: readNoise_In set to " << readNoise_In << endl;
  int I_Telluric=0;
  int I_XCorProf = 0;
  ...
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "TELLURIC")) >= 0){
    I_Telluric = *(int*)parameterValues_In[pos];
    cout << "MExtract::main: I_Telluric set to " << I_Telluric << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SWATH_WIDTH")) >= 0){
    I_SwathWidth = *(int*)parameterValues_In[pos];
    cout << "MExtract::main: I_SwathWidth set to " << I_SwathWidth << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "MAX_ITER_SF")) >= 0){
    I_MaxIterSF = *(int*)parameterValues_In[pos];
    cout << "MExtract::main: I_MaxIterSF set to " << I_MaxIterSF << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "MAX_ITER_SKY")) >= 0){
    I_MaxIterSky = *(int*)parameterValues_In[pos];
    cout << "MExtract::main: I_MaxIterSky set to " << I_MaxIterSky << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "MAX_ITER_SIG")) >= 0){
    I_MaxIterSig = *(int*)parameterValues_In[pos];
    cout << "MExtract::main: I_MaxIterSig set to " << I_MaxIterSig << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SMOOTH_SF")) >= 0){
    D_SmoothSF = *(double*)parameterValues_In[pos];
    cout << "MExtract::main: D_SmoothSF set to " << D_SmoothSF << endl;
    S_A1_Args(3) = "LAMBDA_SF";
    PP_Args[3] = &D_SmoothSF;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SMOOTH_SP")) >= 0){
    I_SmoothSP = *(int*)parameterValues_In[pos];
    cout << "MExtract::main: I_SmoothSP set to " << I_SmoothSP << endl;
    S_A1_Args(4) = "LAMBDA_SP";
    PP_Args[4] = &I_SmoothSP;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "WING_SMOOTH_FACTOR")) >= 0){
    D_WingSmoothFactor = *(double*)parameterValues_In[pos];
  ...
  /// AREA
  sComp = "AREA";
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "AREA")) >= 0){
    I_A1_Area = *(blitz::Array<int, 1>*)parameterValues_In[pos];
    cout << "MExtract: I_A1_Area set to " << I_A1_Area << endl;
    S_A1_Args(2) = "AREA";
    PP_Args[2] = &I_A1_Area;
  ...
  /// 2D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "ERR_IN")) >= 0){
    errorImageIn = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: ERR_IN set to " << errorImageIn << endl;
  }

  /// 2D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "ERR_OUT_2D")) >= 0){
    errorImageOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: ERR_OUT_2D set to " << errorImageOut << endl;
  }

  /// 1D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "ERR_OUT_EC")) >= 0){
    errorOutEc = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: ERR_OUT_EC set to " << errorOutEc << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "ERRFIT_OUT_EC")) >= 0){
    errorFitOutEc = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: ERR_OUT_EC set to " << errorFitOutEc << endl;
  }

  /// 1D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SKY_OUT_EC")) >= 0){
    skyOutEc = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: SKY_OUT_EC set to " << skyOutEc << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SKYFIT_OUT_EC")) >= 0){
    skyFitOutEc = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: SKYFIT_OUT_EC set to " << skyFitOutEc << endl;
  }

  /// 2D
  sComp = "SKY_OUT_2D";
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SKY_OUT_2D")) >= 0){
    skyImageOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: SKY_OUT_2D set to " << skyImageOut << endl;
  }
    
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SKYFIT_OUT_2D")) >= 0){
    skyFitImageOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: SKYFIT_OUT_2D set to " << skyFitImageOut << endl;
  }

  /// 1D
  sComp = "SKY_ERR_OUT_EC";
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SKY_ERR_OUT_EC")) >= 0){
    skyErrorOutEc = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: SKY_ERR_OUT_EC set to " << skyErrorOutEc << endl;
  }
    
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SKYFIT_ERR_OUT_EC")) >= 0){
    skyFitErrorOutEc = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: SKYFIT_ERR_OUT_EC set to " << skyFitErrorOutEc << endl;
  }

  /// 2D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "IM_REC_OUT")) >= 0){
    imageOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: IM_REC_OUT set to " << imageOut << endl;
  }

  /// 2D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "REC_FIT_OUT")) >= 0){
    imageFitOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: REC_FIT_OUT set to " << imageFitOut << endl;
  }

  /// 2D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "PROFILE_OUT")) >= 0){
    profileImageOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: PROFILE_OUT set to " << profileImageOut << endl;
  }

  /// 2D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "MASK_OUT")) >= 0){
    maskOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: MASK_OUT set to " << maskOut << endl;
  }

  /// 1D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "SPFIT_OUT_EC")) >= 0){
    specFitOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: SPFIT_OUT_EC set to " << specFitOut << endl;
  }

  /// 1D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "EC_FROM_PROFILE_OUT")) >= 0){
    specFromProfileOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: EC_FROM_PROFILE_OUT set to " << specFromProfileOut << endl;
  }

  /// 1D
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "ERR_FROM_PROFILE_OUT")) >= 0){
    errorFromProfileOut = *(string*)parameterValues_In[pos];
    cout << "MExtract::main: ERR_FROM_PROFILE_OUT set to " << errorFromProfileOut << endl;
  }

  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "APERTURES")) >= 0){
    apertureListIn = *(string*)parameterValues_In[pos];
  ...
  if ((pos = cf.KeyWord_Set(parameterKeywords_In, "XCOR_PROF")) >= 0){
    I_XCorProf = *(int*)parameterValues_In[pos];
...
//    cout << "MExtract::main: I_SwathWidth set to " << I_SwathWidth << endl;
  ...
  for (int i_file = 0; i_file < S_A1_FitsFileNames_In.size(); i_file++){
    ...
    /// Set ReadOutNoise
    cout << "MExtract::main: Starting F_Image.Set_ReadOutNoise(" << readNoise_In << ")" << endl;
    if (!F_Image.Set_ReadOutNoise( readNoise_In ))
    {
      cout << "MExtract::main: ERROR: F_Image.Set_ReadOutNoise(" << readNoise_In << ") returned FALSE!" << endl;
      exit(EXIT_FAILURE);
    }

    /// Set Gain
    cout << "MExtract::main: Starting F_Image.Set_Gain(" << gain_In << ")" << endl;
    if (!F_Image.Set_Gain( gain_In ))
    {
      cout << "MExtract::main: ERROR: F_Image.Set_Gain(" << gain_In << ") returned FALSE" << endl;
      exit(EXIT_FAILURE);
    }
