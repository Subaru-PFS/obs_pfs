...
/**
function int CountLines(const char fname[255]: inout)
Returns number of lines of file <fname>.
 **/
long MBasis::countlines(const char fname[255])
{
 ...
 **/
double MBasis::calcintens(const double x1, const double x2, const double xn, const double y1, const double y2)
{
  double intensn = 0.;
  intensn = y1;
 ...
 **/
char* MBasis::StrTrimChar(const char* P_Str, const char Char, int Mode)
{
  int i;
  int NewLength = 0;
  int Before = 1;
...
Appends strtoapp at inoutarr[oldlength] and a '\0' behind and returns position of '\0'.
 **/
char* MBasis::StrTrim(const char* P_Str, int Mode)
{
...
Appends strtoapp at inoutarr[oldlength] and a '\0' behind and returns position of '\0'.
 **/
char* MBasis::SubString(const char*P_Str, int Start, int End)
{
  int i, Pos, Length;
  char* P_OutStr;
...
Appends strtoapp at inoutarr[oldlength] and a '\0' behind and returns position of '\0'.
 **/
int MBasis::charrcat(char inoutarr[255], const char *strtoapp, int oldlength)
{
#ifdef __DEBUG_MBASIS__
 ...
 **/
int MBasis::strcatstr(char* inout, const char* strtoapp)
{
#ifdef __DEBUG_MBASIS__
 ...
 **/
int MBasis::charposincharrfrom(const char inarr[255], const char lookfor, int start)
{
  int pos = start;
  do
  {
 ...
 **/
int MBasis::charposincharr(const char inarr[255], const char lookfor)
{
 ...
 **/
int MBasis::lastcharposincharr(const char inarr[255], const char lookfor)
{
  int pos = 0;
  int lastpos = -1;
#ifdef __DEBUG_MBASIS__
...
Returns last position of character 'in' in 'inarr', if found, else '-1'
 **/
int MBasis::strposincharrfrom(const char inarr[255], const char* lookfor, int startpos)
{
  int i;
  int pos = startpos;
#ifdef __DEBUG_MBASIS__
 ...
 **/
int MBasis::strposincharr(const char inarr[255], const char* lookfor)
{
 ...
 **/
int MBasis::read2dfiletoarrays(const char* filename, double* warr, double* varr)
{
...
Returns 1, if successfull, else 0.
 **/
int MBasis::WriteArraysToFile(const char* P_FileName, const double* P_XArray, double* P_YArray, long Start, long End)
{
  FILE *P_File;
  long i;
...
Returns 1, if successfull, else 0.
 **/
void MBasis::Swap(double &A,double &B)
{
  double tmp = A;
...
Returns 1, if successfull, else 0.
 **/
void MBasis::BubbleSort(double *P_Arr,long Size)
{
...
Returns 1, if successfull, else 0.
 **/
void MBasis::ShellSort(double *P_Arr,long Size)
{
  long Step = Size;
  long Start, i, j;
...
Returns 1, if successfull, else 0.
 **/
double MBasis::Mean(const double *P_X, const long XStart, const long XEnd)
{
  double Sum = 0.;
  double Mean;
  long i;
...
Returns 1, if successfull, else 0.
 **/
double* MBasis::CopyDblArray(const double *P_ArrToCopy, long Start, long End)
{
  long i, Pos;
  double *P_OutArr;
...
Returns 1, if successfull, else 0.
 **/
double MBasis::Median(double *P_Arr, long Start, long End)
{
  double *P_TempArr;
 ...
 **/
int MBasis::CharPosInStrFrom(const char* P_InStr, const char LookFor, int Start)
{
  char StrArr[255];
 ...
 **/
int MBasis::CharPosInStr(const char* P_InStr, const char LookFor)
{
  char StrArr[255];
  strcpy(StrArr, P_InStr);
 ...
 **/
int MBasis::IsDouble(const char* P_Str)
{
  char PointChar = '.';
  int Pos;
  int IntArrPos;
 ...
 **/
int MBasis::IsInt(const char* P_Str)
{
  int Pos;
  int IntArrPos;
...
Returns 1, if P_Str is a integer, else 0.
 **/
int MBasis::IsNumber(const char* P_Str)
{
...
Returns first position of String 'P_LookFor' in 'P_InStr', if found, else '-1'
 **/
int MBasis::StrPosInStr(const char* P_InStr, const char* P_LookFor)
{
  char StrArr[255];
  strcpy(StrArr, P_InStr);
 ...
 **/
int MBasis::LastCharPosInStr(const char* P_InStr, const char LookFor)
{
  char StrArr[255];
  strcpy(StrArr, P_InStr);
 ...
 **/
int MBasis::StrPosInStrFrom(const char* P_InStr, const char* P_LookFor, int Start)
{
  char StrArr[255];
  strcpy(StrArr, P_InStr);
#ifdef __DEBUG_MBASIS__
 ...
 **/
int MBasis::IfParameterSet(const char *P_Parameters, int SizeOfParameters, const char *P_ParameterName)
{
  int i;
 ...
 **/
double MBasis::Round(const double ToRound, int DigitsBehindDot)
{
  long TempLong;
  int TempInt;
  double TempDbl;
  ...
  TempDbl = (ToRound * pow(10, DigitsBehindDot)) - TempLong;
  TempInt = (int)(TempDbl * 10);
  if (TempInt > 4)
    TempLong++;
  return (TempLong / pow(10, DigitsBehindDot));
}
 ...
 **/
long MBasis::RoundToLong(const double ToRound)
{
  return (long)Round(ToRound, 0);
}

//int Uniq(char* P_Arr, int Index)
//{
/**$Id: uniq.pro,v 1.8 2001/01/15 22:28:14 scottm Exp $
 ...
 **/
int* MBasis::WhereEqual(const double* P_Arr, const int Size, const double Equal, int* NEqual)
{
 ...
 **/
int* MBasis::WhereGreater(const double* P_Arr, const int Size, const double Min, int* NGreater)
{
  int *P_Indizes;// = (int*)malloc(sizeof(int));
  int i;
 ...
 **/
int* MBasis::WhereLower(const double* P_Arr, const int Size, const double Max, int* NLower)
{
  int *P_Indizes;// = (int*)malloc(sizeof(int));
  int i;
 ...
 **/
double** MBasis::Total(const double*** PP_Arr, const int NDims, const int* P_Size, const int Dimension)
{
  long i, j, k;
...
void* MBasis::Replicate(void* Content, int XSize, int YSize)
{
  //  return void*;
}

char* MBasis::StringToPChar(const string &str)
{
  char *p_char = (char*)malloc(sizeof(char) * 255);
  int i;
  bool EndFound = false;
  while (!EndFound && i < 255)
  {
    p_char[i] = str[i];
    if (p_char[i] == '\0')
      EndFound = true;
...
/*char* MBasis::StringToPChar(const string &str)
{
  char* p_str = (char*)malloc(sizeof(char) * 255);
  int i;
  bool EndFound = false;
  while (!EndFound && i < 255)
  {
...
/**
function int LastCharPosInStr(const string Str: in, const char LookFor: in)
Returns last position of character 'P_LookFor' in 'Str', if found, else '-1'
 **/
int MBasis::LastCharPosInString(const string InStr, const char LookFor)
{
  //  char *p_Str = (char*)malloc(sizeof(char) * 255);
  //  p_Str = StringToPChar(InStr);
  return (LastCharPosInStr(StringToPChar(InStr), LookFor));
}
