...
#include "MFindApsInRing.h"

int main(int argc, char *argv[])
{
  cout << "MFindApsInRing::main: argc = " << argc << endl;
  if (argc < 8)
  {
    cout << "MFindApsInRing::main: ERROR: Not enough parameters specified!" << endl;
    cout << "USAGE: findapsinring <char[] FitsFileName_In> <char[] DatabaseFileName_In> <int x_center> <int y_center> <int inner_radius> <int outer_radius> <char[] FileList_Out>" << endl;
    exit(EXIT_FAILURE);
  }
  ...
  int I_InnerRadius = (int)(atoi((char*)argv[5]));
  int I_OuterRadius = (int)(atoi((char*)argv[6]));
  char *P_CharArr_FileList_Out = (char*)argv[7];
  ...
  {
    cout << "MFindApsInRing::main: ERROR: F_Image.SetFileName(" << CS_FitsFileName_In << ") returned FALSE!" << endl;
    exit(EXIT_FAILURE);
  }
  cout << "MFindApsInRing::main: F_Image: FileName <" << CS_FitsFileName_In << "> set" << endl;
  
  if (!F_Image.ReadArray()){
    cout << "MFindApsInRing::main: ERROR: ReadArray() returned FALSE" << endl;
    exit(EXIT_FAILURE);
  }
  ...
  {
    cout << "MFindApsInRing::main: ERROR: F_Image.SetDatabaseFileName(" << CS_DBFileName_In << ") returned FALSE!" << endl;
    exit(EXIT_FAILURE);
  }
  cout << "MFindApsInRing::main: F_Image: DatabaseFileName <" << CS_DBFileName_In << "> set" << endl;

  /// Read DatabaseFileName_In
  if (!F_Image.ReadDatabaseEntry())
  {
    cout << "MFindApsInRing::main: ERROR: F_Image.ReadDatabaseEntry() returned FALSE!" << endl;
    exit(EXIT_FAILURE);
  }
  cout << "MFindApsInRing::main: F_Image: DatabaseEntry read" << endl;
  
  if (!F_Image.CalcTraceFunctions()){
    cout << "MFindApsInRing::main: ERROR: CalcTraceFunctions returned FALSE" << endl;
  }
  ...
  I_A1_Where = where(D_A1_Radius > double(I_InnerRadius) && D_A1_Radius < double(I_OuterRadius), 1, 0);
  Array<int, 1> *P_I_A1_Index = F_Image.GetIndex(I_A1_Where, I_NInd);
  if (I_NInd < 0){
    cout << "MFindApsInRing::main: ERROR: I_NInd = " << I_NInd << endl;
    exit(EXIT_FAILURE);
  }
  cout << "MFindApsInRing::main: P_I_A1_Index = " << *P_I_A1_Index << endl;
