...
def createFlatFiberTraceProfileSES(filename):
    ...
    msi.findAndTraceApertures(ftffc, 1100, mif.getHeight(), 10)
...
#    msi.getFiberTraceSet().sortTracesByXCenter();
    ...
    msi.getFiberTraceSet().getFiberTrace(20).getImage().writeFits(filename_trace)

    # ---  create profile and extract fiber trace 0
    msi.getFiberTraceSet().getFiberTrace(20).setFiberTraceExtractionControl(ftec)
    msi.getFiberTraceSet().getFiberTrace(20).MkSlitFunc()
    
    # --- get profile and write profile to fits file
#    profile = msi.getFiberTraceSet().getFiberTrace(20).getProfile()
...
#    reconstructed = msi.getFiberTraceSet().getFiberTrace(20).getReconstructed2DSpectrum()
...
#    imMinusRec = msi.getFiberTraceSet().getFiberTrace(20).getImage().getArray() - reconstructed.getArray()
