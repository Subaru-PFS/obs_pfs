...
class H4Config(pexConfig.Config):
    ...
    rateCRRepair = pexConfig.Field(
        dtype=bool, default=True,
        doc="Subtract the CR contribution from the linearized cube. Set False to flag "
            "in the CR mask plane only (useful when manually inspecting CR detection).",
    )
    doDeglitch = pexConfig.Field(
        dtype=bool, default=True,
        doc="Detect (and repair) ASIC glitches alongside CRs in the iterative "
            "detector. Default True: run deglitching on all pixels, all "
            "channels — bright glitches are reliably classified and repaired "
            "even though faint-end performance still has known false-positive/"
            "negative tails. Set False to disable glitch detection entirely; "
            "CRs are still detected. Only meaningful when ``doCR`` is True.",
    )
...
class PfsIsrTask(ipIsr.IsrTask):
    ...
    def makeNirExposure(self,
        ...
        if self.config.h4.quickCDS:
            ...
            deltas = self.makeUTRdeltas(pfsRaw, r0=r0, r1=r1)
            ...
            # makeUTRdeltas zero-anchors at r0, so flux[i] = read[r0+i+1] - read[r0].
            ...
            if self.config.h4.doLinearize and linearity is not None:
                ...
                if r0 > 0 and offsetRaw is not None:
                    ...
                    if nirDark is not None and self.config.h4.applyUTRWeights:
                        ...
                        # offset must match.
                        offsetForLin = offsetRaw - self.getDarkRead(nirDark, r0)
                    else:
                ...
                if self.config.h4.doCR:
                    ...
                    if self.config.h4.doDeglitch:
                        ...
                        glitchChanMask = np.ones(flux.shape[1:], dtype=bool)
                    else:
                    ...
                    iterResult = h4Linearity.cr.iterativeUtrDetectAndRepair(
                        ...
                        repair=self.config.h4.rateCRRepair,
